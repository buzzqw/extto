...
- **DB:** SQLite (tre database separati: `extto_series.db` per lo stato operativo, `extto_archive.db` per la memoria storica, `extto_config.db` per la configurazione — introdotto in v40)
...
**8. Config si rilegge ogni ciclo** — nessun riavvio per modifiche alla configurazione. Da v40, `Config._load()` legge da `extto_config.db` (via `core/config_db.py`). `libtorrent.py` usa ancora `Config()` per coerenza. I file `extto.conf`, `series.txt`, `movies.txt` erano il formato precedente — se ancora presenti vengono migrati automaticamente al primo avvio.
...
| Configurazione / migrazione config | `core/config_db.py`, `core/config.py`, `core/database.py` |
...
- **Auto-gestione Download/Upload** (opt-in, `libtorrent_dynamic_queue=yes` + UI in Configurazione → Libtorrent → "Auto-gestione Download/Upload", rinominata da "Coda Dinamica" perché gestisce anche upload/totali, non solo il download): `_adjust_dynamic_queue()` (ogni 90s) regola insieme `active_downloads` (tra `dynamic_queue_min`/`max`, in base al rate aggregato vs `download_rate_limit` configurato — non una banda "reale" misurata, il tetto che imposta l'utente), `active_seeds` (scende a 1 quando c'è un download prioritario in corso, stabile richiesta ~180s prima di cambiare per non fermare/riavviare i seed ogni giro) e `active_limit` (sempre `max(statico, downloads+seeds+2)`, e non scende mai sotto il valore corrente a meno che downloads o seeds non stiano scendendo anche loro in quel giro — altrimenti diventerebbe lui stesso un collo di bottiglia invisibile). Nessun valore viene mai persistito nel DB: al riavvio del ciclo (~2h) si riparte dai tre valori statici configurati. `_allocate_torrent_resources()` distribuisce connessioni/slot upload/banda per-torrent pesati per tier invece del pool globale condiviso senza criterio, e riduce gli slot upload dei seed quando c'è un download prioritario in corso. `_reset_torrent_resource_overrides()` ripristina i default quando il toggle viene disattivato.
- **UI**: nuova sezione "Auto-gestione Download/Upload" in `templates/index.html` (toggle + slot min/max, tooltip esteso) e campi corrispondenti in `static/js/app.js` (`saveLtSettings()`, `saveAllConfig()`, caricamento form). Quando il toggle è attivo, `toggleDynamicQueue()` disabilita in UI (e mostra un hint) i tre campi statici "Max download/upload attivi"/"Max totali", che restano solo il punto di partenza usato a ogni riavvio ciclo.
- **Nota YAML i18n**: chiavi di traduzione oltre 1024 caratteri (i tooltip lunghi) NON possono essere scritte come `'chiave': 'valore'` su una riga sola in `languages/*.yaml` — YAML richiede la forma esplicita `? chiave` / `: valore` su due righe oltre quella soglia, e chiavi con `: ` (due punti + spazio) al loro interno vanno sempre quotate. Generare queste righe con `yaml.safe_dump({k: v}, allow_unicode=True, default_style="'")` invece di scriverle a mano evita l'errore "mapping values are not allowed here".
...
- **`core/i18n_db.py`** (NUOVO): gestisce tabelle `i18n_translations` (chiave/lingua/valore) e `i18n_settings` (lingua attiva) in `extto_config.db`. Funzioni: `get/set_ui_language()`, `get_translation()`, `set_translation_bulk()`, `delete_translation_lang()`, `get_languages()`.
- **`languages/it.yaml`** (NUOVO): 830 chiavi master italiano (chiave=valore). Importabile da UI → Manutenzione → Lingue & Traduzioni.
- **`languages/en.yaml`** (AGGIORNATO): da 427 a 723 chiavi inglesi. Copre tutte le stringhe UI dinamiche (`app.js`) e statiche (`index.html`).
- **`index.html`** (AGGIORNATO): ~400 attributi i18n aggiunti (`data-i18n`, `data-i18n-title`, `data-i18n-placeholder`). Nessuna stringa italiana hardcoded.
- **`app.js`** (AGGIORNATO): tutti i `showToast()`, `confirm()`, `alert()`, `innerHTML` template e label dinamiche usano `t('chiave')`. `_primaryLang` sostituisce ogni fallback `|| 'ita'`. Fix `saveSeriesChanges()`/`saveMovieChanges()` per lingua Personalizzato.
- **`extto_web.py`** (AGGIORNATO): nuovi endpoint `/api/i18n/*`, `/api/series/all-missing`, `/api/series/calendar`, `/api/scan-all-archives`. Aggiunto `_config_write_lock`. Fix `request.json or {}` su 7 endpoint. Connessioni SQLite convertite a context manager.
- **BONUS_ITA rimosso**: la lingua è filtro obbligatorio/preferito, non bonus di scoring. Score identico per ITA e ENG a parità di qualità tecnica.

### v40 — Migrazione Configurazione a DB (17 Marzo 2026)

- **`core/config_db.py`** (NUOVO): gestisce `extto_config.db` con tabelle `settings` (chiave/valore) e `movies_config`. Migrazione automatica da `extto.conf`/`series.txt`/`movies.txt` al primo avvio. Wizard UI in pagina Configurazione con pulsanti "Importa nel DB" e "Rinomina in .old".
- **`core/config.py`**: `_load()` riscritto per leggere da `extto_config.db`. Interfaccia pubblica identica (`cfg.series`, `cfg.movies`, `cfg.qbt` ecc.) — zero impatto su `extto3.py`.
- **`core/database.py`**: tabella `series` estesa con colonne `seasons`, `language`, `enabled`, `archive_path`, `timeframe`, `ignored_seasons`, `tmdb_id`, `subtitle`. `sync_configs()` fa upsert completo.
- **`extto_web.py`**: `parse/save_series_config` e `parse/save_movies_config` riscritte per leggere/scrivere sul DB. 3 nuovi endpoint: `GET /api/config/migration-status`, `POST /api/config/migrate`, `POST /api/config/rename-old`.
- **Fix parse_quality** (`models.py`): `[DV HDR10]`, `[h265]`, `[IT]` ora riconosciuti da titoli rinominati da extto.
- **Colonna Aggiunto** nella lista episodi delle serie TV (data invio al client).
- **Cloud Backup**: rimossi Google Drive, OneDrive, Dropbox (non mantenuti). Rimasto solo FTP. Aggiunto checkbox "Invia ZIP via Telegram al termine".
- **Fix browser cartelle**: `openDirBrowser` usava `showEl()` (classe `hidden`) invece di `classList.add('active')` → modal non si apriva. `z-index: 3000` sul dir-browser-modal per apparire sopra gli altri modal. `event.stopPropagation()` sui pulsanti sfoglia per evitare chiusura del modal parent.
- **Fix eliminazione serie**: `_save_series_to_db` non faceva DELETE. Aggiunto `sync_delete=True` opzionale, attivato solo da `deleteSeriesFromConfig`.
- **Fix log torrent**: `remove_torrent` ora logga il nome del torrent invece dell'hash `.fastresume`.
- **`fix_seasons.py`**: script one-shot per correggere `seasons` e `archive_path` nel DB dopo la migrazione.
...
## v39b - Download Mega via megatools + fix Pulisci (13 Marzo 2026)
...
- **Supporto Cloud Backup Esteso**: Aggiunta configurazione per FTP e Dropbox nella sezione Manutenzione. *(Nota: Google Drive, OneDrive e Dropbox rimossi in v40 — rimasto solo FTP + invio Telegram)*
